...
public class Week005_MyRenderPipeline : RenderPipeline {
	...
	public class CameraRenderer {
		...
		Camera m_camera;   // why need to store variable in class 
		CullingResults m_cullingResults; //This data includes information about visible objects, lights, and reflection probes. 
...
// https://docs.unity3d.com/Packages/com.unity.render-pipelines.universal@11.0/manual/urp-shaders/urp-shaderlab-pass-tags.html#urp-pass-tags-lightmode
		...
		public void Render (ScriptableRenderContext context, Camera camera, Week005_MyRenderPipelineAsset pipelineAsset) {
			...
			m_cmd.name = "Week005_MyRenderPipeline";//update render function

			m_context.SetupCameraProperties(m_camera);  // unity_MatrixVP
										//STEP1			//clear the render target to get rid of its old contents.
														// From 1 to 4 they are Skybox, Color, Depth  and Nothing.
			bool clearDepth = m_camera.clearFlags >= CameraClearFlags.Depth; //why >=  Nothing
			bool clearColor = m_camera.clearFlags >= CameraClearFlags.Color; //why >=,  Depth, and Nothing.
			m_cmd.ClearRenderTarget(clearDepth, clearColor, clearColor ? m_camera.backgroundColor.linear : Color.clear); // clear the color buffer  ,in the case of Skybox we end up replacing all previous color data 
			...
			//STEP2 // culling those that fall outside of the view frustum of the camera.
			if (camera.TryGetCullingParameters(out var cullParam)) { // var equivalent to write a separate line to define local variable? // out not copy, reference
				m_cullingResults = context.Cull(ref cullParam); //This data includes information about visible objects, lights, and reflection probes.
			   //				Render_Unlit_Opaque();
        ...
        void Render_GBuffer_Setup()
        {
            ...
            m_cmd.name = "Render_GBuffer"; // why need this name

            var desc = new RenderTextureDescriptor(m_camera.pixelWidth, m_camera.pixelHeight, RenderTextureFormat.Default, 0);  //struct contains all the information required to create a RenderTexture.
			
			
			//open three texture
			desc.colorFormat = RenderTextureFormat.Default; m_cmd.GetTemporaryRT(GBufferIds.baseColor, desc, FilterMode.Point); // why use point
            ...
            m_cmd.SetRenderTarget(s_GBufferTargetBinding); //Gbuffertargetbinding 
        ...
        {
            var _ProjectionParams = new Vector4(1, m_camera.nearClipPlane, 1 / m_camera.farClipPlane);// why 1/clip plan , why need this projection params
            ...
            m_cmd.SetGlobalInt(s_MyGBufferCull, _MyGBufferCull); // s id
            m_cmd.SetGlobalVector(s_ProjectionParams, _ProjectionParams);

            var sortingSettings = new SortingSettings(m_camera)
            {
                criteria = SortingCriteria.CommonOpaque //If something ends up drawn behind something else its hidden fragments can be skipped, which speeds up rendering.
			};
            var filteringSettings = new FilteringSettings(RenderQueueRange.opaque)
            {
                layerMask = m_camera.cullingMask
            };
            var drawingSettings = new DrawingSettings(ShaderTags.MyGBuffer, sortingSettings); //my Gbuffer layer 
            DrawRenderers(ref drawingSettings, ref filteringSettings);
        }
        ...
        public struct GBufferIds {
			public static readonly int baseColor   = Shader.PropertyToID("MyGBuffer_baseColor"); // IDs in GPU //why only assign this three property
			public static readonly int positionWS  = Shader.PropertyToID("MyGBuffer_positionWS");  // IDs in GPU
			public static readonly int normalWS    = Shader.PropertyToID("MyGBuffer_normalWS");  // IDs in GPU
		}

		static readonly RenderTargetBinding s_GBufferTargetBinding = new RenderTargetBinding() { // why need this binding 
		...
		public static readonly int s_MyGBufferCull    = Shader.PropertyToID("_MyGBufferCull"); //readyonly 
		

		Shader   m_GBufferLigthingShader = Shader.Find("Week005/Week005_Deferred_Phong");
		Material m_GBufferLigthingMaterial;

		public static readonly int s_scaledScreenParams  = Shader.PropertyToID("_ScaledScreenParams"); // why need to assdign proport 
		public static readonly int s_MyGBuffer_DebugMode = Shader.PropertyToID("_MyGBuffer_DebugMode");

		void Render_GBuffer_Ligthing() {
			m_cmd.name = "Render_GBuffer_Ligthing"; // why need to assign name 
			m_cmd.SetRenderTarget(BuiltinRenderTextureType.CameraTarget);

			if (!m_GBufferLigthingMaterial) {
				m_GBufferLigthingMaterial = new Material(m_GBufferLigthingShader);// open new material to assign lighting 
			}
